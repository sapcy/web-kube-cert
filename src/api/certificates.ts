 ...
 */
export function generateKubernetesPKI(request: KubeCertRequest): GeneratedCerts {
  ...
  const apiServerKubeletClient = generateClientCert(
    kubernetesCA,
...
function generateEtcdCerts(
  ...
  // etcd 서버 인증서용 SAN
  const etcdSANs: Array<{ type: 'dns' | 'ip'; value: string }> = [
 ...
 */
function buildAPIServerSANs(
  clusterName: string,
  apiServerAddress: string,
  serviceCIDR: string,
  additionalSANs: string
): Array<{ type: 'dns' | 'ip'; value: string }> {
  const sans: Array<{ type: 'dns' | 'ip'; value: string }> = []
