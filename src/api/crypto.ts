 ...
 * RSA 키 쌍 생성 (2048 bit)
 */
export function generateKeyPair(): forge.pki.rsa.KeyPair {
...
export function generateServerCert(
  ...
  commonName: string,
  validityDays: number,
  altNames: Array<{ type: 'dns' | 'ip'; value: string }>
 ...
 */
export function generatePeerCert(
  ca: CAInfo,
  commonName: string,
  validityDays: number,
  altNames: Array<{ type: 'dns' | 'ip'; value: string }>
 ...
 */
export function generateServiceAccountKeyPair(): { publicKey: string; privateKey: string } {
  const keys = generateKeyPair()
  return {
 ...
 * IP 주소를 바이트로 변환
 */
function ipToBytes(ip: string): string {
 ...
 */
export function getFirstIPFromCIDR(cidr: string): string {
 ...
 * SAN 문자열 파싱
 */
export function parseSANs(sans: string): Array<{ type: 'dns' | 'ip'; value: string }> {
  if (!sans.trim()) return []
