 ...
 * 인증서 파일들을 ZIP으로 압축
 */
export async function createCertZip(files) {
    const zip = new JSZip();
    // kubernetes 폴더 구조 생성
 ...
 * PKI 전용 README.md 생성
 */
function generatePKIReadme(files) {
 ...
 * PKI 전용 설치 스크립트
 */
function generatePKIInstallScript() {
    return `#!/bin/bash
 ...
 * 전체 파일용 README.md 생성
 */
function generateFullReadme(files) {
    const certFiles = files.filter((f) => f.type === 'certificate');
    const keyFiles = files.filter((f) => f.type === 'key');
 ...
 * 전체 파일용 설치 스크립트
 */
function generateFullInstallScript() {
    return `#!/bin/bash
set -e
