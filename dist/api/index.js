 ...
 */
export async function generateKubeCerts(request) {
    // 입력 검증
    ...
    if (request.includeKubeconfig) {
        const kubeconfigFiles = generateKubeconfigs(pkiResult.kubernetesCA, request.apiServerAddress, request.clusterName, request.certDays);
 ...
 * ZIP 파일 다운로드
 */
export function downloadZip(blob, filename = 'kubernetes-pki.zip') {
